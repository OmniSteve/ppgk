...
export default function PlayerCreditSelector({ selected, onSelect }) {
  ...
  if (selected) {
    return (
      <div className="bg-[#2563EB]/10 border border-[#2563EB]/30 rounded-xl p-4 space-y-2">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-lg bg-[#2563EB]/20 flex items-center justify-center flex-shrink-0">
              <User size={16} className="text-[#2563EB]" />
            </div>
            <div>
              <p className="font-bold text-white text-sm">{selected.playerName}</p>
              <p className="text-slate-400 text-xs">Parent: {selected.clientName}</p>
            </div>
          </div>
          <button onClick={handleClear} className="text-slate-400 hover:text-white transition-colors" aria-label="Clear selection">
            <X size={16} />
          </button>
        </div>
  ...
  return (
    <div ref={containerRef} className="relative">
      <label className="block text-slate-400 text-xs mb-1">Search player / parent</label>
      ...
      {showResults && touched && query.trim().length >= 2 && (
        <div className="absolute z-30 mt-1 w-full bg-[#1e2330] border border-white/10 rounded-xl shadow-xl max-h-72 overflow-y-auto">
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-slate-400 text-xs truncate">Parent: {p.clientName}</p>
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-slate-500 mt-0.5">
                      {p.clientEmail && <span className="truncate">{p.clientEmail}</span>}
                      {p.clientPhone && <span>{p.clientPhone}</span>}
                    </div>
                  </div>
                  <span className="flex-shrink-0 text-xs font-semibold px-2 py-1 rounded-full bg-[#2563EB]/20 text-[#60A5FA]">
