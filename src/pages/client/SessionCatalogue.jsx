...
const SessionCard = ({ session, onSelect, selected }) => (
  <div
    className={`bg-white/5 rounded-2xl border transition-all cursor-pointer ${
      selected ? 'border-[#2563EB] ring-1 ring-[#2563EB]/30' : 'border-white/10 hover:border-[#2563EB]/40'
    }`}
    ...
    <div className="p-5">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-white text-sm leading-tight">{session.name}</h3>
            <p className="text-slate-400 text-xs">{session.sessionType}</p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <Link to={`/sessions/${session.id}`} onClick={(e) => e.stopPropagation()} className="text-[#2563EB] text-xs font-semibold hover:underline">
          Details →
        </Link>
      </div>
    </div>
    {selected && (
      <div className="border-t border-[#2563EB]/20 bg-[#2563EB]/10 px-5 py-2.5 flex items-center gap-2">
...
const inputCls = 'bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#2563EB] transition-colors';
...
export default function SessionCatalogue() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div>
        <h1 className="text-2xl font-black text-white">Available Sessions</h1>
        <p className="text-slate-400 text-sm mt-0.5">Select one or more sessions to book</p>
      </div>
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-4 space-y-3">
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all ${showFilters ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'border-white/10 text-slate-400 hover:border-[#2563EB] hover:text-white'}`}
          >
      ...
      {loading ? (
        ...
        <div className="bg-white/5 rounded-2xl border border-white/10 p-16 text-center">
          ...
          <p className="text-slate-400 font-medium">No sessions available</p>
          <p className="text-slate-500 text-sm mt-1">Check back soon or adjust your filters</p>
        </div>
      ) : (
      ...
      {selected.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 lg:left-64 bg-[#0D1B2A] border-t border-white/10 px-4 py-4 flex items-center justify-between z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={proceedToCheckout} className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold px-6 py-2.5 rounded-xl text-sm transition-colors">
