    ...
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Manage goalkeeper profiles</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center">
          ...
          <p className="text-slate-500 font-medium mb-4">No players yet</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {players.map((p) => (
            <div key={p.id} className="bg-white rounded-2xl border border-slate-200 p-5 hover:border-[#2563EB]/40 hover:shadow-sm transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${p.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                  {p.status}
                </span>
              </div>
              <div className="space-y-1.5 mb-4">
                {p.currentClub && (
                  <p className="text-slate-500 text-xs flex items-center gap-1.5"><Activity size={11} />Club: {p.currentClub}</p>
                )}
                <p className="text-slate-500 text-xs flex items-center gap-1.5">
                  <Calendar size={11} />
                  DOB: {p.dateOfBirth ? new Date(p.dateOfBirth).toLocaleDateString('en-MT') : '—'}
                </p>
              </div>
