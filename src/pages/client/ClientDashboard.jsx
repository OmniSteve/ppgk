...
const StatCard = ({ label, value, sub, icon: Icon, color, href }) => (
  <Link to={href || '#'} className="bg-white rounded-2xl p-5 border border-slate-200 hover:border-[#2563EB]/40 hover:shadow-md transition-all group">
...
const BookingCard = ({ booking }) => (
  <Link to={`/bookings/${booking.id}`} className="flex items-center gap-4 p-4 rounded-xl bg-white border border-slate-200 hover:border-[#2563EB]/40 hover:shadow-sm transition-all">
    ...
    <div className="flex-1 min-w-0">
      <p className="font-semibold text-slate-900 text-sm truncate">{booking.sessionName || 'Training Session'}</p>
      <p className="text-slate-500 text-xs mt-0.5 flex items-center gap-1">
        <Clock size={11} />
        {booking.startTime} – {booking.endTime}
        {booking.locationName && <><span className="text-slate-300">·</span><MapPin size={11} />{booking.locationName}</>}
      </p>
    </div>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Here's your training overview</p>
        </div>
        <Link
          to="/sessions"
          className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors flex items-center gap-2"
        >
          <Plus size={16} />
          Book Session
        </Link>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          ...
          sub={data?.expiringCredits > 0 ? `${data.expiringCredits} expiring soon` : 'All valid'}
          icon={CreditCard}
      ...
      {data?.expiringCredits > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-amber-800 font-semibold text-sm">{data.expiringCredits} credit{data.expiringCredits > 1 ? 's' : ''} expiring soon</p>
            <p className="text-amber-700 text-xs">Use them before they expire.</p>
          </div>
          <Link to="/credits" className="text-amber-700 text-sm font-semibold hover:underline">View →</Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {[
          ...
          <Link key={a.href} to={a.href} className="bg-white border border-slate-200 hover:border-[#2563EB]/40 rounded-xl p-4 flex items-center gap-3 hover:shadow-sm transition-all group">
