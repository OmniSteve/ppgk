...
import { Plus, Package, Edit2, ToggleLeft, ToggleRight, Loader2 } from 'lucide-react';
...
export default function PackageManagement() {
  ...
  const openNew = () => { setForm(defaultPkg); setEditing('new'); setError(''); };
  const openEdit = (pkg) => { setForm({ ...pkg }); setEditing(pkg.id); setError(''); };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing === 'new') {
        ...
        await apiClient.put(`/admin/packages/${editing}`, form);
      }
      setEditing(null);
      load();
    } catch (err) {
  ...
  const inputCls = "w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#2563EB] transition-colors";
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openNew} className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-2">
          <Plus size={16} />
          New Package
        </button>
      </div>
      ...
      {editing && (
        <div className="bg-white/5 rounded-2xl border border-[#2563EB]/30 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Package Name</label><input value={form.name} onChange={set('name')} className={inputCls} placeholder="e.g. Monthly Bundle" /></div>
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Credits</label><input type="number" value={form.credits} onChange={set('credits')} className={inputCls} placeholder="e.g. 8" /></div>
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Price (EUR)</label><input type="number" step="0.01" value={form.price} onChange={set('price')} className={inputCls} placeholder="0.00" /></div>
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Validity (months)</label><input type="number" value={form.validityMonths} onChange={set('validityMonths')} className={inputCls} /></div>
          </div>
          <div><label className="block text-slate-300 text-xs font-medium mb-1">Description</label><textarea value={form.description} onChange={set('description')} className={inputCls + ' resize-none'} rows={2} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Eligible Session Types</label><input value={form.eligibleSessionTypes} onChange={set('eligibleSessionTypes')} className={inputCls} placeholder="All (leave blank for all)" /></div>
            <div><label className="block text-slate-300 text-xs font-medium mb-1">Eligible Locations</label><input value={form.eligibleLocations} onChange={set('eligibleLocations')} className={inputCls} placeholder="All (leave blank for all)" /></div>
          </div>
          ...
          {packages.map((pkg) => (
            <div key={pkg.id} className="flex items-center gap-4 px-5 py-4 hover:bg-white/5 transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => openEdit(pkg)} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-[#2563EB] flex items-center justify-center text-slate-400 hover:text-white transition-all">
                  <Edit2 size={14} />
                </button>
              </div>
            </div>
