  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setForm(defaultPkg); setEditing('new'); setError(''); }} className="bg-primary hover:bg-primary-hover text-foreground font-bold px-4 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-2xl border border-border divide-y divide-border">
          {packages.map((pkg) => (
            <div key={pkg.id} className="flex items-center gap-4 px-5 py-4 hover:bg-accent transition-colors">
              <div className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center flex-shrink-0">
                <Package size={18} className="text-primary" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-foreground text-sm">{pkg.name}</p>
                <p className="text-muted-foreground text-xs text-label-mono">{pkg.credits} credits · €{pkg.price} · {pkg.validityMonths} months</p>
              </div>
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${pkg.active ? 'bg-success/20 text-success' : 'bg-accent text-muted-foreground'}`}>
                {pkg.active ? 'Active' : 'Inactive'}
              </span>
              <div className="flex items-center gap-2">
                <button onClick={() => toggleActive(pkg)} className="w-8 h-8 rounded-lg bg-accent hover:bg-accent/70 flex items-center justify-center text-muted-foreground hover:text-foreground transition-all">
                  {pkg.active ? <ToggleRight size={16} /> : <ToggleLeft size={16} />}
                </button>
                <button onClick={() => { setForm({ ...pkg }); setEditing(pkg.id); setError(''); }} className="w-8 h-8 rounded-lg bg-accent hover:bg-primary flex items-center justify-center text-muted-foreground hover:text-foreground transition-all">
                  <Edit2 size={14} />
                </button>
              </div>
            </div>
