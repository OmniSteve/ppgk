    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      {session && (
        <div className="bg-[#0D1B2A] rounded-2xl p-5 border border-white/10">
          ...
          <p className="text-slate-400 text-sm mt-1">{session.sessionDate ? new Date(session.sessionDate).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' }) : '—'} · {session.startTime}</p>
        </div>
      )}
