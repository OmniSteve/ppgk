  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Your assigned sessions</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Today's Sessions", value: data?.todaySessions?.length ?? 0, icon: Calendar, color: 'bg-[#2563EB]/10 text-[#2563EB]' },
          { label: 'Upcoming Sessions', value: data?.upcomingSessions?.length ?? 0, icon: Clock, color: 'bg-green-100 text-green-700' },
          { label: 'Attendance Pending', value: data?.attendancePending ?? 0, icon: ClipboardList, color: 'bg-amber-100 text-amber-700' },
        ].map((s) => (
      ...
      {data?.todaySessions?.length > 0 && (
        <div>
          <h2 className="font-bold text-slate-900 mb-3">Today's Sessions</h2>
            ...
            {data.todaySessions.map((s) => (
              <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div className="flex gap-2">
                  <Link to={`/coach/sessions/${s.id}/attendees`} className="flex-1 text-center text-sm font-semibold py-2 border border-slate-200 rounded-xl text-slate-700 hover:border-[#2563EB]/40 transition-all flex items-center justify-center gap-2">
                    <Users size={14} />
                    Attendees
                  </Link>
      ...
      <div>
        ...
        {data?.upcomingSessions?.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-500 text-sm">No upcoming sessions assigned</p>
          </div>
        ) : (
          <div className="space-y-2">
            {data.upcomingSessions.slice(0, 5).map((s) => (
              <Link key={s.id} to={`/coach/sessions/${s.id}/attendees`} className="flex items-center gap-4 p-4 bg-white rounded-xl border border-slate-200 hover:border-[#2563EB]/40 transition-all">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-500 text-xs">{s.startTime} · {s.locationName} · {s.bookingCount} players</p>
                </div>
              </Link>
            ))}
          </div>
