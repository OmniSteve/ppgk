          ...
          {sessions.map((s) => (
            <div key={s.id} className="bg-card rounded-2xl border border-border p-5 hover:border-primary/40 transition-all">
              <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                ...
                <span className="font-bold text-sm bg-accent text-foreground px-3 py-1 rounded-full text-label-mono flex-shrink-0">{s.bookedCount}/{s.capacity}</span>
              </div>
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 mb-4 text-muted-foreground text-xs">
                ...
                <span className="flex items-center gap-1.5 min-w-0 truncate"><MapPin size={12} className="flex-shrink-0" />{s.locationName}</span>
