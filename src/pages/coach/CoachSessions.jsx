    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex gap-2">
        {[{ key: 'upcoming', label: 'Upcoming' }, { key: 'today', label: 'Today' }, { key: 'past', label: 'Past' }].map((f) => (
          <button key={f.key} onClick={() => { setFilter(f.key); setLoading(true); }} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filter === f.key ? 'bg-[#2563EB] text-white' : 'bg-white/5 border border-white/10 text-slate-400 hover:border-[#2563EB]/40 hover:text-white'}`}>
            {f.label}
          </button>
        ))}
      </div>
